...
```mermaid
graph TD
    A["Déficit neurológico subagudo en adulto joven"] --> B{"¿Dura ≥ 24 h, sin fiebre ni infección?"}
    B -->|"NO"| C["PSEUDOBROTE: buscar infección, calor, estrés. No corticoide"]
    B -->|"SÍ"| D["Evento desmielinizante probable (CIS)"]
    D --> E{"¿Rasgos de alarma: neuritis bilateral o grave, mielitis ≥ 3 segmentos, área postrema?"}
    E -->|"SÍ"| F["Sospecha NMOSD / MOGAD: AQP4-IgG y MOG-IgG (base celular)"]
    E -->|"NO"| G["RM cerebral y medular con gadolinio"]
    G --> H{"¿DIS: lesiones en ≥ 2 regiones típicas?"}
    H -->|"NO"| I["CIS sin criterios: seguimiento con RM"]
    H -->|"SÍ"| J{"¿DIT: realce simultáneo, lesión nueva o bandas oligoclonales?"}
    J -->|"SÍ"| K["ESCLEROSIS MÚLTIPLE (si no hay mejor explicación)"]
    J -->|"NO"| L["Punción lumbar: bandas oligoclonales / RM de control"]
```
