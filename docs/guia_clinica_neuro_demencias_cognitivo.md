...
```mermaid
graph TD
    A["Queja Cognitiva / Falla de Memoria"] --> B{"¿Es fluctuante y agudo con inatención?"}
    B -->|"SÍ"| C["DELIRIUM: Buscar infección, fármacos, metabólico"]
    B -->|"NO"| D{"¿Afecta la autonomía funcional (IADL)?"}
    D -->|"Autonomía conservada"| E["Deterioro Cognitivo Leve (MCI) / Queja Subjetiva"]
    D -->|"Dependencia en actividades diarias"| F["DETERIORO NEUROCOGNITIVO MAYOR (DEMENCIA)"]
    
    F --> G["Descarte Reversible: TSH, B12, VDRL, RM cerebral"]
    G --> H{"¿Marcha apráxica + Incontinencia urinaria precoz?"}
    H -->|"SÍ"| I["Sospecha Hidrocefalia Normotensiva -> Tap Test"]
    H -->|"NO"| J{"Identificar primer dominio comprometido"}
    
    J -->|"Memoria episódica anterógrada"| K["Enfermedad de Alzheimer"]
    J -->|"Escalonado + Focalidad + Vascular"| L["Demencia Vascular"]
    J -->|"Fluctuaciones + Alucinaciones + Parkinsonismo"| M["Demencia por Cuerpos de Lewy"]
    J -->|"Conducta desinhibida / Apatía / Lenguaje"| N["Degeneración Frontotemporal"]
```
